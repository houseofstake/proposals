...
**Last Call** - Final review period before voting. This status may be shortened or skipped entirely. The community has a final opportunity to provide feedback. Substantive changes during Last Call move the proposal back to Review.

**Voting** - The proposal is being voted on by House of Stake voters.
...
**Rejected** - The proposal was voted on and did not pass.
...
5. **Last Call** - An editor moves the proposal to Last Call, setting a 14-day deadline. This is the final opportunity for community feedback before voting.

6. **Voting** - Proposal proceeds to an on-chain vote by veNEAR holders. Voting period is 14 days (this is a parameter on the [voting contract](https://nearblocks.io/address/vote.dao) which can only be changed by the security council). The proposal must meet quorum and approval thresholds defined in the House of Stake governance documents.

7. **Final/Rejected** - Based on vote results, the proposal becomes Final or Rejected.
...
Note: the HSP author should not arbitrarily choose a HSP number. The initial proposal should include:

```
hsp: TBA
```

An HSP editor will assign the HSP number, and is responsible for updating the frontmatter, when merging a draft HSP.
